...
void exibeJogo(const Tetris &jogoComPecaCaindo,int larguraJogo,int alturaMaximaJogo) {
	...
	for(int i=0;i<alturaMaximaJogo;i++) {
		// printando a borda mais à esquerda
		mvprintw(i, 0, "|");
		for(int j=0;j<larguraJogo;j++) {
			// printando o tabuleiro atual
			mvprintw(i, j+1, "%c",jogoComPecaCaindo.get(j,alturaMaximaJogo-i-1));
		}
		// printando a borda mais à direita
...
int main() {
	...
	while(true) {		
		...
		// Movimentação da peça para a esquerda e direita
 		if(ultimaTecla==KEY_LEFT) { 	

 			Tetris jogoTeste = jogoComPecaCaindo;
			 // Vejo se eu consigo criar uma peça à esquerda da posição atual
 			if(jogoTeste.adicionaForma(posicaoPecaAtual-1,alturaPecaAtual,idPecaAtual, possiveisRotacoes[rotacaoPecaAtual]))
 				posicaoPecaAtual--; // Se sim, vou me mover para a esquerda
 		} else if(ultimaTecla==KEY_RIGHT){
	
 			Tetris jogoTeste = jogoComPecaCaindo; 			
			 // Vejo se eu consigo me mover à direita da posição atual
 			if(jogoTeste.adicionaForma(posicaoPecaAtual+1,alturaPecaAtual,idPecaAtual, possiveisRotacoes[rotacaoPecaAtual]))
 				posicaoPecaAtual++;// se sim,  vou me mover para a direita
 		} else if(ultimaTecla==' '){ // Se eu apertar espaço rotaciona a peça
 			Tetris jogoTeste = jogoComPecaCaindo; 	
		...
		// Peça "caindo"
		// Se eu não cheguei no final do tabuleiro ou não colidi com nenhuma peça, diminui a sua altura(peça caindo)
		if(jogoComPecaCaindo.adicionaForma(posicaoPecaAtual,alturaPecaAtual-1,idPecaAtual, possiveisRotacoes[rotacaoPecaAtual])) {
			alturaPecaAtual--;
		}
		else {	// Se não, cria outra peça
		...
		exibeJogo(jogoComPecaCaindo,larguraJogo,alturaMaximaJogo);
 		...
 		//usleep(200000);
		 //cout << "peca " << jogoComPecaCaindo.get(6, 19) << "\n";
